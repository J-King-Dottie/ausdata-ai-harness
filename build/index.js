...
function isCustomDomesticDataset(datasetId) {
    return String(datasetId || "").trim().startsWith("CUSTOM_AUS,");
}
function normalizeAnchorType(dimensionId, conceptId) {
    ...
    if (["CAT", "CATEGORY", "SUPG", "SUPC", "PRODUCT", "COMMODITY", "INDUSTRY", "SECTOR", "FLOW"].some((token) => text.includes(token))) {
        return "CATEGORY";
    }
    return "";
}
...
function rawMetadataPayload(datasetId, metadata) {
    ...
    const orderedDimensions = [...dimensions].sort((left, right) => Number(left?.position || 0) - Number(right?.position || 0));
    ...
    for (const dimension of orderedDimensions) {
        ...
        const conceptName = String(concept?.name || concept?.description || conceptId || dimensionId).trim() || dimensionId;
    ...
    const wildcardTemplateFor = (anchorDimensionId) => {
        const parts = dimensionOrder.map((dimensionId) => dimensionId === anchorDimensionId ? `{${dimensionId}}` : "");
        return parts.length > 0 ? parts.join(".") : "all";
    };
    const anchorCandidatesByType = new Map();
    ...
    const anchorCandidates = Array.from(anchorCandidatesByType.values()).sort((left, right) => anchorPriority(String(right.anchor_type || "")) - anchorPriority(String(left.anchor_type || "")));
    return {
        kind: "raw_metadata",
...
function validateAnchorWildcardDataKey(datasetId, dataKey) {
    const cleanDataKey = String(dataKey || "").trim();
    if (!cleanDataKey || cleanDataKey.toLowerCase() === "all") {
        throw new Error(`Invalid raw ABS dataKey. ABS retrieval must follow metadata-derived anchor selection; broad 'all' retrieval is not allowed. Received datasetId=${datasetId}, dataKey=${dataKey}.`);
    }
    const segments = cleanDataKey.split(".");
    const fixedSegments = segments
        .map((segment, index) => [index, String(segment || "").trim()])
        .filter(([, token]) => token);
    if (fixedSegments.length !== 1) {
        throw new Error(`Invalid raw ABS dataKey. raw_retrieve must use exactly one anchored segment and wildcard every other segment. Received datasetId=${datasetId}, dataKey=${dataKey}.`);
    }
    const [, anchorToken] = fixedSegments[0];
    if (anchorToken.includes("+")) {
        throw new Error(`Invalid raw ABS dataKey. raw_retrieve must use exactly one anchor code, not multiple codes in one segment. Received datasetId=${datasetId}, dataKey=${dataKey}.`);
    }
}
function buildWildcardDataKey(metadataPayload, anchorType, anchorCode) {
    ...
    const target = candidates.find((item) => {
        ...
        return String(item.anchor_type || "").trim().toUpperCase() === anchorType;
    });
    if (!target) {
    ...
    const isAllowed = allowedCodes.some((item) => {
        ...
        return String(item.code || "").trim() === cleanAnchorCode;
    });
    if (!isAllowed) {
        throw new Error(`Invalid ABS anchor code '${cleanAnchorCode}' for anchorType=${anchorType}. Choose a code from the metadata anchor_candidates list.`);
    }
...
function artifactPathForId(artifactId) {
    return path.join(runtimeDir, "conversations", conversationId, "artifacts", `${artifactId}.json`);
}
async function storeDomesticArtifact(payload, label) {
    if (!conversationId) {
...
async function loadArtifactPayload(artifactId) {
    const artifactPath = artifactPathForId(artifactId);
...
async function latestDomesticArtifactId() {
    if (!conversationId) {
        return undefined;
    }
...
function flattenDomesticPayload(payload) {
    ...
    for (const series of seriesItems) {
        ...
        for (const observation of observations) {
            ...
            const obsAttrs = typedObservation.attributes && typeof typedObservation.attributes === "object"
                ? typedObservation.attributes
                : {};
            const row = [typedSeries.seriesKey];
...
function domesticPreviewRows(payload, limit = 8) {
...
function domesticArtifactManifest(artifactId, kind, label, summaryText, payload, extra = {}) {
...
function matchesTimeRange(value, start, end) {
...
function writeCsvText(headers, rows) {
    const lines = [];
    if (headers.length) {
...
function estimateAnalysisCsvBytes(headers, rows) {
    if (!headers.length) {
        return 0;
    }
    return Buffer.byteLength(writeCsvText(headers, rows), "utf8");
}
async function uploadAnalysisFile(artifactId, label, headers, rows) {
    ...
    if (csvBytes > MAX_ANALYSIS_UPLOAD_BYTES) {
        throw new Error(`Analysis file exceeds upload limit: ${(csvBytes / (1024 * 1024)).toFixed(2)}MB > 50MB.`);
    }
...
async function artifactFileSizeBytes(artifactId) {
...
const server = new Server({
    ...
    version: "0.1.0",
    description: "Access Australian domestic public data, including ABS and curated custom Australian sources. Preferred workflow: shortlist the dataset, inspect metadata when needed, retrieve, inspect the stored artifact, narrow only if needed, then analyze."
}, {
    capabilities: {
        tools: {
            list: true,
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
        tools: [
            {
                name: "search_catalog",
                description: "Search the merged Australian domestic catalog across ABS and curated custom Australian sources. Use this first unless the dataset is already clear.",
                inputSchema: {
                    type: "object",
                    properties: {
                        forceRefresh: {
            ...
            {
                name: "list_dataflows",
                description: "List available domestic dataflows with identifiers and descriptions. Prefer search_catalog when the user is asking for a dataset by topic rather than by known id.",
                inputSchema: {
                    type: "object",
                    properties: {
                        forceRefresh: {
            ...
            {
                name: "get_metadata",
                description: "Fetch structural metadata for a domestic dataset, using source-specific adapters where needed. For ABS datasets, do this before retrieve so you can choose one anchor from anchor_candidates and let the server construct the wildcard retrieval correctly.",
                inputSchema: {
                    type: "object",
                    required: ["datasetId"],
                    properties: {
                        datasetId: {
                            type: "string",
                            description: "Domestic dataset identifier in {agencyId},{dataflowId},{version} format (e.g., ABS,CPI,1.1.0 or CUSTOM_AUS,RBA_A1,1.0)"
                        },
                        forceRefresh: {
                            type: "boolean",
            ...
            },
            {
                ...
                description: "Fetch domestic dataset observations and return normalized data using source-specific metadata and adapters",
                inputSchema: {
                    type: "object",
                    required: ["datasetId"],
                    properties: {
                        ...
                        dataKey: {
                            type: "string",
                            description: "Optional SDMX data key for slicing the dataset (e.g., dots-separated dimension codes)"
                        },
                        startPeriod: {
                            type: "string",
                        ...
                        format: {
                            ...
                            description: "Optional ABS format (defaults to jsondata for lightweight queries; resolver uses metadata)"
                        },
                        dimensionAtObservation: {
                            type: "string",
                            description: "Optional ABS dimensionAtObservation parameter (default TIME_PERIOD for time-series)"
                        },
                        detail: {
                            type: "string",
            ...
            {
                name: "retrieve",
                description: "Retrieve a domestic dataset using the correct source adapter behind one domestic tool contract. For ABS datasets, do this only after get_metadata, use exactly one metadata-derived anchor, and do not invent ad hoc ABS dataKey input.",
                inputSchema: {
                    type: "object",
                    required: ["datasetId"],
                    properties: {
                        datasetId: {
                            type: "string",
                            description: "Domestic dataset identifier in {agencyId},{dataflowId},{version} format (e.g., ABS,CPI,1.1.0 or CUSTOM_AUS,AES_TABLE_O,1.0)"
                        },
                        dataKey: {
                            type: "string",
                            description: "Optional source-specific retrieval key. For ABS this is the SDMX data key; for custom sources it may be 'all' or a curated slice."
                        },
                        anchorType: {
                            type: "string",
                            description: "ABS only. One metadata-derived anchor type such as DATA_ITEM, MEASURE, or CATEGORY. Use with anchorCode so the server constructs the wildcard dataKey."
                        },
                        anchorCode: {
                            type: "string",
                            description: "ABS only. One metadata-derived anchor code chosen from get_metadata.anchor_candidates. The server will construct the wildcard dataKey."
                        },
                        startPeriod: {
                            type: "string",
            ...
            },
            {
                ...
                description: "Inspect a stored domestic retrieval artifact and return a compact structural summary plus preview rows, including size information that helps decide whether to use it directly or narrow it first. Prefer inspect before analysis.",
                inputSchema: {
                    type: "object",
                    properties: {
                        artifactId: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    try {
        ...
        logger.info("Domestic MCP tool start", {
            conversationId,
            tool: name,
            args: summarizeArgs(args ?? undefined),
        });
        switch (name) {
            case "search_catalog":
            case "list_dataflows": {
                const forceRefresh = typeof args?.forceRefresh === "boolean" ? args.forceRefresh : false;
            ...
            case "get_dataflow_metadata": {
                ...
                const metadata = await dataFlowService.getDataStructureForDataflow(args.datasetId, typeof args?.forceRefresh === "boolean" ? args.forceRefresh : false);
            ...
            case "query_dataset": {
                ...
                if (!isCustomDomesticDataset(args.datasetId)) {
                    throw new Error("ABS query_dataset is not allowed. For ABS datasets, use get_metadata first and then retrieve with anchorType + anchorCode.");
                }
                const data = await dataFlowService.getFlowData(args.datasetId, typeof args?.dataKey === "string" ? args.dataKey : "all", {
                    ...
                    detail: typeof args?.detail === "string"
                        ? args.detail
                        : undefined,
                    dimensionAtObservation: typeof args?.dimensionAtObservation === "string"
            ...
            case "resolve_dataset": {
                ...
                let dataKey = typeof args.dataKey === "string" && args.dataKey.trim()
                    ? args.dataKey.trim()
                    : undefined;
                if (!isCustomDomesticDataset(args.datasetId)) {
                    if (dataKey) {
                        throw new Error("ABS retrieve does not accept dataKey input. Use get_metadata first, choose one anchor from anchor_candidates, and call retrieve with anchorType + anchorCode.");
                    }
                    ...
                    if (!anchorType || !anchorCode) {
                        throw new Error("ABS retrieve requires anchorType + anchorCode from get_metadata. ABS always uses metadata-derived anchor + wildcard retrieval.");
                    }
                    const metadata = await dataFlowService.getDataStructureForDataflow(args.datasetId, typeof args?.forceRefresh === "boolean" ? args.forceRefresh : false);
                ...
                const result = await datasetResolver.resolve({
                    ...
                    dimensionAtObservation: typeof args.dimensionAtObservation === "string"
                        ? args.dimensionAtObservation
                        : undefined,
                    format: typeof args.format === "string"
                        ? args.format
                        : undefined,
                    forceRefresh: typeof args.forceRefresh === "boolean" ? args.forceRefresh : undefined,
                });
                const dataset = result.dataset && typeof result.dataset === "object"
                    ? result.dataset
                    : {};
                const label = (typeof dataset.name === "string" && dataset.name.trim()) ||
                    (typeof dataset.id === "string" && dataset.id.trim()) ||
                    args.datasetId;
                const manifest = await storeDomesticArtifact(result, String(label));
                logger.info("Domestic MCP tool success", {
                    conversationId,
                    tool: name,
                    duration_ms: Date.now() - startedAt,
            ...
            case "inspect_artifact": {
                const requestedArtifactId = typeof args?.artifactId === "string" && args.artifactId.trim()
                ...
                const label = String((payload.dataset && typeof payload.dataset === "object"
                    ? payload.dataset.name
                    : "") || requestedArtifactId).trim() || requestedArtifactId;
                const kind = String(payload.kind || (requestedArtifactId.startsWith("narrowed-domestic-") ? "domestic_narrowed" : "domestic_retrieve")).trim();
                const extra = {};
                ...
                const manifest = domesticArtifactManifest(requestedArtifactId, kind, label, `Inspected domestic artifact '${label}'.`, payload, extra);
                logger.info("Domestic MCP tool success", {
                    conversationId,
                    tool: name,
                    duration_ms: Date.now() - startedAt,
            ...
            case "narrow_artifact": {
                ...
                const maxSeries = typeof args?.maxSeries === "number" && Number.isFinite(args.maxSeries)
                    ? Math.max(1, Math.min(Math.floor(args.maxSeries), 40))
                    : 12;
                const noExplicitFilters = Object.keys(dimensionFilters).length === 0 &&
                ...
                if (matrixStylePayload && noExplicitFilters) {
                    throw new Error("For supply-use, input-output, or matrix-style domestic tables, narrow_artifact requires a specific metric/anchor filter so it can isolate one correct full matrix before python analysis.");
                }
                ...
                if (noExplicitFilters && kind === "domestic_narrowed" && sourceSeries.length <= maxSeries) {
                    ...
                    const manifest = domesticArtifactManifest(requestedArtifactId, kind, label, `Narrowed domestic artifact '${label}'.`, payload, analysis);
                    logger.info("Domestic MCP tool success", {
                        conversationId,
                        tool: name,
                        duration_ms: Date.now() - startedAt,
                ...
                for (const series of sourceSeries) {
                    ...
                    for (const observation of observations) {
                        ...
                        const timeValue = String(typedObservation.observationKey ||
                            (obsDims.TIME_PERIOD?.label) ||
                            (obsDims.TIME_PERIOD?.code) ||
                            "").trim();
                ...
                if (estimatedBytes > MAX_ANALYSIS_UPLOAD_BYTES) {
                    throw new Error(`Narrowed artifact is still too large for python handoff (${(estimatedBytes / (1024 * 1024)).toFixed(2)}MB > 50MB). Narrow further to one correct full matrix or one metric/anchor.`);
                }
                const analysis = await uploadAnalysisFile(artifactId, `${label} narrowed`, rows.headers, rows.rows);
                const manifest = domesticArtifactManifest(artifactId, "domestic_narrowed", `${label} (narrowed)`, `Narrowed domestic artifact '${label}'.`, narrowedPayload, {
